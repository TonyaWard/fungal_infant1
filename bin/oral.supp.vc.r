##Birth mode using anal and oral
...
######################################################################
#Plot candida albicans on day 30 in oral
map2 <- mapping[mapping$SuperbodysiteOralSkinNoseVaginaAnalsAureola == "Oral",]
candida <- ggplot(map2, aes(y=Candida_orthopsilosis, x=Delivery_Vvaginal_Ccs_IcsInoc, fill=Delivery_Vvaginal_Ccs_IcsInoc)) +
  geom_boxplot(outlier.shape = NA) +
  theme_cowplot(font_size=7) +
  geom_jitter(position=position_jitter(0.1), shape=1, size=1) +
  guides(fill=FALSE) +
  labs(y = "Candida orthopsilosis (relative abundance)", x= "Birth Mode") +
  scale_fill_manual(values=c("#c3c823", "#053d58")) +
  annotate("text", x="V", y=1.0, label= paste("P<0.001"), size=2)
...
pdf(paste(main_fp, "/Supplement_Oral_VC.pdf", sep=""), width=6.64, height=3)
plot(final_plot)
dev.off()
